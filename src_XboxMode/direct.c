...
void ReadDB9(report_t *reportBuffer)
{
	...
	if ((PIND & 0b00110000) == 0)		// if L/R low then Megadrive pad detected
	{
		...
		if (!(PINB & (1<<0))) reportBuffer->b2 |= (1<<3);	// Megadrive Start
	}
	...
	if (!(PIND & (1<<3))) reportBuffer->hat |= HAT_DOWN;	// down
	if (!(PIND & (1<<4))) reportBuffer->hat |= HAT_LEFT;	// left
	if (!(PIND & (1<<5))) reportBuffer->hat |= HAT_RIGHT;	// right
...
void ReadDB15(report_t *reportBuffer)
{
	...
	if (!(PINB & (1<<1))) reportBuffer->hat |= HAT_DOWN;		// Down
