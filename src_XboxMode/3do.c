...
void Read3DO(report_t *reportBuffer)
{
	...
	if (temp & (1<<0))	reportBuffer->hat |= HAT_LEFT;	// left
	if (temp & (1<<1))	reportBuffer->hat |= HAT_RIGHT; // right
	if (temp & (1<<2))	reportBuffer->hat |= HAT_UP;	// up
	if (temp & (1<<3))	reportBuffer->hat |= HAT_DOWN;	// down
