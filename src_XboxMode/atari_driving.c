	...
	if (cur != last)
	{
		switch (cur)
		{
			case 0b00:
				...
				if (last == 0b01) reportBuffer->hat |= HAT_RIGHT;	// right
				if (last == 0b10) reportBuffer->hat |= HAT_LEFT;	// left
				break;
			case 0b01:
				//if (last == 0b11) reportBuffer->x = 127;
				//if (last == 0b00) reportBuffer->x = -128;
				if (last == 0b11) reportBuffer->hat |= HAT_RIGHT;	// right
				if (last == 0b00) reportBuffer->hat |= HAT_LEFT;	// left
				break;
			case 0b11:
				//if (last == 0b10) reportBuffer->x = 127;
				//if (last == 0b01) reportBuffer->x = -128;
				if (last == 0b10) reportBuffer->hat |= HAT_RIGHT;	// right
				if (last == 0b01) reportBuffer->hat |= HAT_LEFT;	// left
				break;
			case 0b10:
				//if (last == 0b00) reportBuffer->x = 127;
				//if (last == 0b11) reportBuffer->x = -128;
				if (last == 0b00) reportBuffer->hat |= HAT_RIGHT;		// right
				if (last == 0b11) reportBuffer->hat |= HAT_LEFT;		// left
				break;
		}
		last = cur;
		bounce = 3;
		pos = reportBuffer->hat;
	}
	else
	{
		if (bounce > 0)
