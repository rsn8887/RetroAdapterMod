...
void ReadPCE(report_t *reportBuffer)
{
	...
	for (i = 2; i; --i)
	{
		...
		if (PINB & (PCE_UP|PCE_DN|PCE_LF|PCE_RT))
		{
			...
			if (!(PINB & PCE_UP)) reportBuffer->hat |= HAT_UP;	// up
			if (!(PINB & PCE_RT)) reportBuffer->hat |= HAT_RIGHT;	// right
			if (!(PINB & PCE_DN)) reportBuffer->hat |= HAT_DOWN;	// down
			if (!(PINB & PCE_LF)) reportBuffer->hat |= HAT_LEFT;	// left			
			
			PORTB &= ~PCE_DS;						// DS low
			asm("nop");
			asm("nop");

			if (!(PINB & PCE_UP)) reportBuffer->b1 |= (1<<0);	// I / A (rightmost button)
			if (!(PINB & PCE_RT)) reportBuffer->b1 |= (1<<1);	// II / B (leftmost button)
			if (!(PINB & PCE_DN)) reportBuffer->b2 |= (1<<2);	// Select
			if (!(PINB & PCE_LF)) reportBuffer->b2 |= (1<<3);	// Run / Start
		}
		else
		{
