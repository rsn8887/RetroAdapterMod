...
void ReadPSX(report_t *reportBuffer, reportAnalogButtons_t *reportBufferAnalogButtons)
{
	...
	if (id==PSX_ID_NEGCON) 
	{
		...
		data = PSXCommand(0xff);
		if (!(data & (1<<3))) reportBuffer->b1 |= (1<<7);	// R1
		if (!(data & (1<<4))) reportBuffer->b1 |= (1<<3);	// /\ Triangle (B on Negcon)
		if (!(data & (1<<5))) reportBuffer->b1 |= (1<<1 );	// O  Circle (A on Negcon)
		...
		// MiSTer PSX core in Wheel-NegCon Mode uses analog stick axes for analog buttons)
		data = PSXCommand(0xff); //I button (bottom button analog)
		reportBuffer->y = -(char)data / 2;
		
		data = PSXCommand(0xff); //II button (left button analog)
		reportBuffer->ry = -(char)data / 2;

		data = PSXCommand(0xff); //L Button
		reportBuffer->rx = -(char)data / 2;
	}
#ifdef PS2PRESSURE
