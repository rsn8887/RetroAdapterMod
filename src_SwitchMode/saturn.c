...
void ReadSaturn(report_t *reportBuffer)
{
	...
	if (PIND & SAT_LF)		// Standard digital pad
	{
		...
		if (!(PINB & SAT_UP)) reportBuffer->hat |= HAT_UP; // up
		if (!(PINB & SAT_DN)) reportBuffer->hat |= HAT_DOWN;			// down
		if (!(PIND & SAT_LF)) reportBuffer->hat |= HAT_LEFT;		// left
		if (!(PIND & SAT_RT)) reportBuffer->hat |= HAT_RIGHT;			// right
